...
func (suite *AuthControllerTestSuite) TestLogin_Success() {
	...
	expectedToken := "fake-jwt-token-123"
	loginCreateModel := model.LoginCreate{
		Email:    "test@example.com",
		Password: "password123",
	}
	suite.mockAuthService.On("Login", loginCreateModel).Return(expectedToken, nil)

	req := httptest.NewRequest(http.MethodPost, "/login", bytes.NewReader(jsonData))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	ctx := suite.echo.NewContext(req, rec)
	err := suite.authController.Login(ctx)
...
func (suite *AuthControllerTestSuite) TestLogin_InvalidCredentials() {
	...
	suite.mockAuthService.On("Login", model.LoginCreate{
		Email:    "test@example.com",
		Password: "wrong-password",
	}).Return("", errors.New("invalid credentials")) // ✅ HATA DÖNSÜN
	...
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), 400, rec.Code)
	assert.Contains(suite.T(), response["error_description"], "invalid credentials")
	suite.mockAuthService.AssertExpectations(suite.T())
}
